    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm transition-all">
      <div className="w-full max-w-lg rounded-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] bg-white text-gray-900 dark:bg-[#1e1e1e] dark:text-white">
        <div className="px-6 py-4 border-b flex justify-between items-center border-gray-100 dark:border-[#333]">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-[#333] transition-colors"
          >
            <FaTimes />
          </button>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 p-3 rounded-xl border outline-none focus:border-blue-500 transition-colors bg-gray-50 border-gray-200 text-gray-900 dark:bg-[#252525] dark:border-[#333] dark:text-white"
                />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-10 p-3 rounded-xl border outline-none focus:border-blue-500 transition-colors appearance-none bg-gray-50 border-gray-200 text-gray-900 dark:bg-[#252525] dark:border-[#333] dark:text-white"
                >
...
export default function AdminUserDetailPage() {
  ...
  const getSocialUrl = (input: string, domain: string) =>
    buildUrl(input, domain as "facebook" | "instagram");
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto font-sans bg-gray-50 text-gray-900 dark:bg-[#121212] dark:text-white">
        ...
        <div className="px-6 sm:px-10 pb-8 relative">
          <div className="flex flex-col sm:flex-row gap-6 items-start -mt-12 sm:-mt-16 mb-4">
            <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full p-1 bg-white dark:bg-[#1e1e1e] shadow-2xl relative shrink-0">
              ...
              <div
                className={`absolute bottom-0 right-0 px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border shadow-md ${profile?.role === "admin" ? "bg-red-500 text-white border-red-400" : "bg-blue-500 text-white border-blue-400"}`}
              >
                {profile?.role || "USER"}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
            ...
            <a
              href={getSocialUrl(profile?.facebook_link || "", "facebook.com")}
              target="_blank"
              className={`p-4 rounded-2xl border flex items-center gap-4 transition-all group ${profile?.facebook_link ? "bg-blue-50 border-blue-100 hover:border-blue-300 dark:bg-blue-900/10 dark:border-blue-900/30 dark:hover:border-blue-600/50 cursor-pointer" : "bg-gray-50 border-gray-100 opacity-60 cursor-not-allowed dark:bg-[#252525] dark:border-[#333]"}`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold text-blue-700 dark:text-blue-300 truncate">
                  {profile?.facebook_link
                    ? extractHandle(profile.facebook_link, "facebook")
                    : "Not Linked"}
                </p>
              </div>
              {profile?.facebook_link && (
                <FaExternalLinkAlt className="text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity text-xs" />
              )}
            </a>
            <a
              href={getSocialUrl(
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold text-pink-700 dark:text-pink-300 truncate">
                  {profile?.instagram_link
                    ? extractHandle(profile.instagram_link, "instagram")
                    : "Not Linked"}
                </p>
              </div>
              {profile?.instagram_link && (
                <FaExternalLinkAlt className="text-pink-400 opacity-0 group-hover:opacity-100 transition-opacity text-xs" />
              )}
            </a>
          </div>
        </div>
      ...
      {tickets.length === 0 ? (
        <div className="text-center py-20 rounded-3xl border-2 border-dashed bg-white border-gray-200 dark:bg-[#1e1e1e] dark:border-[#333]">
